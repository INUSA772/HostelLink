...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ph-dist-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; max-width: 1100px; margin: 0 auto; }
  .ph-dist-card {
    background: white; border: 1.5px solid var(--light-border); border-radius: var(--radius);
    padding: 1.5rem 1rem; text-align: center; color: var(--dark);
    display: flex; flex-direction: column; align-items: center; gap: 0.5rem;
    transition: all 0.25s; box-shadow: 0 2px 8px rgba(0,0,0,0.05);
  }
  .ph-dist-card:hover { background: var(--teal); border-color: var(--teal); color: white; transform: translateY(-5px); box-shadow: 0 12px 30px rgba(26,92,82,0.25); }
  .ph-dist-card i { font-size: 1.7rem; color: var(--teal); transition: color 0.25s; margin-bottom: 0.3rem; }
  .ph-dist-card:hover i { color: white; }
  .ph-dist-card h4 { font-size: 0.92rem; font-weight: 700; }
  .ph-dist-count { font-size: 0.7rem; font-weight: 700; color: var(--teal); background: var(--teal-light); border-radius: 20px; padding: 2px 10px; transition: all 0.25s; }
  .ph-dist-card:hover .ph-dist-count { background: rgba(255,255,255,0.25); color: white; }
...
function BrowseDrawer({ filter, filterValue, filterIcon, onClose, allProperties }) {
  ...
  useEffect(() => {
    setLoading(true);
    // If we already have all properties passed in, filter client-side
    ...
    // Otherwise fetch from API with filter
...
function DistrictsSection({ allProperties }) {
  ...
  useEffect(() => {
    if (allProperties && allProperties.length > 0) {
      const c = {};
      allProperties.forEach(p => { if (p.district) c[p.district] = (c[p.district] || 0) + 1; });
      setCounts(c);
      return;
    }
  ...
  return (
    <>
      <section className="ph-dist-sec" id="browse-districts">
        ...
        <p className="ph-sec-sub">Click your district to instantly see all available properties — no sign-up needed.</p>
        <div className="ph-dist-grid">
          {DISTRICT_CARDS.map(d => (
            <button key={d.label} className="ph-dist-card" onClick={() => setDrawer(d)}>
              <i className={d.icon} />
              <h4>{d.label}</h4>
...
export default function Home() {
  ...
  // Pre-fetch all properties once so district counts and browsing are instant
  useEffect(() => {
    fetch(`${API_URL}/properties?limit=500`)
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <>
      ...
      {/* Drawer triggered from photo grid */}
      {locDrawer && (
        <BrowseDrawer
          filter="district"
