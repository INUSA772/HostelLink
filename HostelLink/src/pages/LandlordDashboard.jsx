...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --shadow-lg:    0 12px 36px rgba(15,25,35,0.14);
    --transition:   all 0.2s ease;
  }

  html { font-size: 16px; -webkit-text-size-adjust: 100%; }
  body { font-family: var(--font); background: var(--off-white); color: var(--dark); min-height: 100vh; }
  ...
  .ld-topbar {
    ...
    padding: 0 2rem;
    box-shadow: 0 2px 16px rgba(15,25,35,0.35);
  }
  .ld-topbar-left { display: flex; align-items: center; gap: 1rem; }
  .ld-logo { display: flex; align-items: center; gap: 0.6rem; text-decoration: none; }
  .ld-logo-icon {
    width: 36px; height: 36px; border-radius: 8px; overflow: hidden;
    border: 2px solid var(--amber); flex-shrink: 0; background: white;
  }
  .ld-logo-icon img { width: 100%; height: 100%; object-fit: contain; display: block; }
  .ld-logo-name { font-size: 1.1rem; font-weight: 800; color: var(--white); letter-spacing: -0.3px; }
  .ld-logo-badge {
  ...
  .ld-topbar-right { display: flex; align-items: center; gap: 0.5rem; }
  .ld-topbar-btn {
    display: flex; align-items: center; gap: 0.4rem;
    padding: 0.42rem 1rem; border-radius: 8px;
    border: 1.5px solid rgba(255,255,255,0.15);
    background: rgba(255,255,255,0.07);
    color: rgba(255,255,255,0.82); font-size: 0.83rem; font-weight: 600;
  ...
  .ld-topbar-btn.danger:hover { background: rgba(220,38,38,0.2); color: #fca5a5; }
  .ld-hamburger {
  ...
  .ld-drawer-close {
    ...
    padding: 0.4rem; border-radius: 6px; transition: var(--transition);
  }
  .ld-drawer-close:hover { background: rgba(255,255,255,0.14); color: white; }
  .ld-drawer-user {
    padding: 1.4rem; border-bottom: 1px solid rgba(255,255,255,0.07);
  ...
  .ld-drawer-uname { font-size: 0.92rem; font-weight: 800; color: white; }
  .ld-drawer-uemail { font-size: 0.7rem; color: rgba(255,255,255,0.38); margin-top: 2px; }
  .ld-drawer-role {
    font-size: 0.63rem; font-weight: 700; color: var(--amber);
    background: rgba(245,166,35,0.15); padding: 2px 7px; border-radius: 10px;
    display: inline-block; margin-top: 4px; text-transform: uppercase; letter-spacing: 0.5px;
  }
  ...
  .ld-drawer-link {
    ...
    padding: 0.8rem 1.5rem; color: rgba(255,255,255,0.58);
  ...
  .ld-drawer-link:hover { background: rgba(255,255,255,0.05); color: white; }
  .ld-drawer-link.active { background: rgba(245,166,35,0.1); color: white; border-left-color: var(--amber); }
  .ld-drawer-link svg { font-size: 0.88rem; width: 17px; flex-shrink: 0; }
  .ld-drawer-link.active svg { color: var(--amber); }
  .ld-drawer-foot { padding: 1rem 1.25rem; border-top: 1px solid rgba(255,255,255,0.07); }
  .ld-drawer-logout {
    display: flex; align-items: center; gap: 0.7rem;
    color: #fca5a5; font-size: 0.88rem; font-weight: 700; cursor: pointer;
    background: rgba(220,38,38,0.1); border: 1px solid rgba(252,165,165,0.18);
    width: 100%; padding: 0.7rem 1rem; border-radius: 9px; transition: var(--transition);
  }
  .ld-drawer-logout:hover { background: rgba(220,38,38,0.18); }
  ...
  .ld-banner h1 em { font-style: normal; color: var(--amber); }
  ...
  .ld-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none !important; }
  .ld-btn-primary {
    background: var(--amber); color: var(--navy);
    box-shadow: 0 4px 16px rgba(245,166,35,0.35);
  }
  .ld-btn-primary:hover:not(:disabled) { background: var(--amber-dark); transform: translateY(-1px); box-shadow: 0 6px 20px rgba(245,166,35,0.4); }
  .ld-btn-ghost {
    background: rgba(255,255,255,0.09); color: white;
    border: 1.5px solid rgba(255,255,255,0.2);
  }
  .ld-btn-ghost:hover:not(:disabled) { background: rgba(255,255,255,0.16); }
  .ld-btn-danger { background: var(--danger); color: white; box-shadow: 0 4px 12px rgba(220,38,38,0.28); }
  .ld-btn-danger:hover:not(:disabled) { opacity: 0.9; transform: translateY(-1px); }
  .ld-btn-outline { background: white; color: var(--navy); border: 1.5px solid var(--border); }
  .ld-btn-outline:hover:not(:disabled) { border-color: var(--amber); color: var(--amber-dark); }
  ...
  .ld-auth-loading {
    ...
    flex-direction: column; gap: 1rem; background: var(--off-white);
  }
  .ld-auth-loading p { color: var(--mid); font-weight: 600; }
  .ld-loading { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 5rem 2rem; gap: 1rem; }
  .ld-spinner { animation: spin 0.8s linear infinite; font-size: 2rem; color: var(--amber); }
  ...
  .ld-stat {
    ...
    padding: 1.4rem 1.5rem; border: 1.5px solid var(--border);
    box-shadow: var(--shadow-sm); display: flex; align-items: center; gap: 1rem;
    transition: var(--transition); position: relative; overflow: hidden;
  }
  .ld-stat::before {
    content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 4px;
    background: var(--amber); transform: scaleY(0); transition: var(--transition);
    transform-origin: bottom; border-radius: 0 2px 2px 0;
  }
  .ld-stat:hover { box-shadow: var(--shadow-md); transform: translateY(-3px); border-color: var(--amber); }
  ...
  .ico-amber  { background: var(--amber-light); color: var(--amber-dark); }
  .ico-navy   { background: var(--navy-pale); color: var(--navy-mid); }
  .ico-green  { background: var(--success-pale); color: var(--success); }
  .ico-mid    { background: var(--light-gray); color: var(--mid); }
  .ld-stat-num { font-size: 1.85rem; font-weight: 900; color: var(--navy); line-height: 1; margin-bottom: 0.3rem; }
  .ld-stat-lbl { font-size: 0.72rem; font-weight: 700; color: var(--mid); text-transform: uppercase; letter-spacing: 0.5px; }
  ...
  .ld-sec-hd h2 svg { color: var(--amber); }

  /* ── QUICK ACTIONS ── */
  .ld-quick { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 2rem; }
  .ld-quick-card {
  ...
  .ld-quick-card:hover { border-color: var(--amber); box-shadow: var(--shadow-md); transform: translateY(-4px); }
  .ld-quick-ico {
  ...
  .ld-quick-card:hover .ld-quick-ico { background: var(--amber); color: var(--navy); transform: scale(1.05); }
  .ld-quick-title { font-size: 0.88rem; font-weight: 800; color: var(--navy); }
  .ld-quick-sub { font-size: 0.74rem; color: var(--mid); font-weight: 500; }
  ...
  .ld-panel-head {
    ...
    background: var(--off-white); flex-wrap: wrap; gap: 0.75rem;
  }
  .ld-panel-label {
    font-size: 0.92rem; font-weight: 800; color: var(--navy);
    display: flex; align-items: center; gap: 0.5rem;
  }
  .ld-panel-label svg { color: var(--amber); }
  .ld-count-pill {
  ...
  .ld-search-box {
    ...
    border-radius: 9px; padding: 0.5rem 0.9rem; min-width: 210px; transition: var(--transition);
  }
  .ld-search-box:focus-within { border-color: var(--amber); box-shadow: 0 0 0 3px rgba(245,166,35,0.12); }
  .ld-search-box input {
    border: none; outline: none; font-size: 0.875rem; color: var(--dark);
    width: 100%; background: transparent; font-family: var(--font);
  }
  .ld-search-box svg { color: var(--mid); flex-shrink: 0; }
  ...
  .ld-hcard:hover { box-shadow: var(--shadow-lg); transform: translateY(-4px); border-color: var(--amber); }
  .ld-hcard-img { position: relative; height: 180px; overflow: hidden; background: var(--light-gray); flex-shrink: 0; }
  .ld-hcard-img img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.4s ease; display: block; }
  .ld-hcard:hover .ld-hcard-img img { transform: scale(1.05); }
  .ld-hcard-img-placeholder {
    width: 100%; height: 100%; display: flex; align-items: center;
    justify-content: center; color: var(--mid); font-size: 2.5rem; opacity: 0.25;
  }
  .ld-hcard-status {
  ...
  .ld-hcard-body { padding: 1.1rem 1.25rem; flex: 1; }
  .ld-hcard-name { font-size: 1rem; font-weight: 800; color: var(--navy); margin-bottom: 0.3rem; line-height: 1.3; }
  .ld-hcard-addr { font-size: 0.78rem; color: var(--mid); display: flex; align-items: center; gap: 0.3rem; margin-bottom: 0.85rem; }
  .ld-hcard-addr svg { color: var(--amber-dark); flex-shrink: 0; }
  .ld-hcard-stats { display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.5rem; margin-bottom: 0.85rem; }
  .ld-hcard-stat { background: var(--off-white); border-radius: 9px; padding: 0.55rem 0.5rem; text-align: center; border: 1px solid var(--border); }
  .ld-hcard-stat-val { font-size: 0.95rem; font-weight: 800; color: var(--navy); line-height: 1; }
  .ld-hcard-stat-lbl { font-size: 0.6rem; font-weight: 700; color: var(--mid); text-transform: uppercase; letter-spacing: 0.3px; margin-top: 0.2rem; }
  .ld-hcard-foot {
    display: flex; align-items: center; justify-content: space-between;
    padding: 0.85rem 1.25rem; border-top: 1.5px solid var(--border);
    background: var(--off-white);
  }
  .ld-hcard-price { font-size: 1rem; font-weight: 800; color: var(--navy); }
  .ld-hcard-price span { font-size: 0.7rem; font-weight: 600; color: var(--mid); }
  .ld-hcard-foot-actions { display: flex; gap: 0.4rem; }
  .ld-foot-btn {
    display: flex; align-items: center; gap: 0.35rem;
    padding: 0.4rem 0.85rem; border-radius: 7px; font-size: 0.77rem; font-weight: 700;
  ...
  .ld-table { width: 100%; border-collapse: collapse; min-width: 600px; }
  .ld-table th {
    background: var(--off-white); padding: 0.85rem 1.1rem; text-align: left;
    font-size: 0.7rem; font-weight: 800; color: var(--navy);
    text-transform: uppercase; letter-spacing: 0.6px;
    border-bottom: 2px solid var(--border); white-space: nowrap;
  }
  .ld-table td { padding: 1rem 1.1rem; border-bottom: 1px solid var(--border); font-size: 0.875rem; vertical-align: middle; }
  .ld-table tbody tr:last-child td { border-bottom: none; }
  .ld-table tbody tr:hover { background: var(--off-white); }
  .ld-cell-name { font-weight: 800; color: var(--navy); font-size: 0.88rem; }
  .ld-badge { display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.28rem 0.75rem; border-radius: 20px; font-size: 0.7rem; font-weight: 700; }
  .ld-badge-verified { background: var(--success-pale); color: #065f46; }
  .ld-badge-pending  { background: var(--amber-light); color: var(--amber-dark); }
  .ld-row-actions { display: flex; gap: 0.4rem; }
  .ld-icon-btn {
    width: 32px; height: 32px; border-radius: 7px; border: 1.5px solid var(--border);
    background: white; cursor: pointer; display: flex; align-items: center;
    justify-content: center; color: var(--mid); font-size: 0.8rem; transition: var(--transition);
  }
  ...
  .ld-empty-ico {
    width: 76px; height: 76px; background: var(--amber-light); border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-size: 2rem; color: var(--amber-dark); margin: 0 auto 1.25rem;
  }
  .ld-empty h3 { font-size: 1.2rem; font-weight: 900; color: var(--navy); margin-bottom: 0.5rem; }
  .ld-empty p { color: var(--mid); font-size: 0.88rem; margin-bottom: 1.5rem; max-width: 300px; margin-left: auto; margin-right: auto; }
  ...
  .ld-modal-icon {
    width: 58px; height: 58px; background: #fef2f2; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    margin: 0 auto 1rem; color: var(--danger); font-size: 1.4rem;
  }
  .ld-modal h3 { font-size: 1.1rem; font-weight: 900; color: var(--navy); margin-bottom: 0.5rem; text-align: center; }
  ...
  .ld-refresh-btn:hover:not(:disabled) { border-color: var(--amber); color: var(--amber-dark); background: var(--amber-light); }
  .ld-refresh-btn:disabled { opacity: 0.45; cursor: not-allowed; }
  .spin { animation: spin 0.8s linear infinite; }

  .ld-view-toggle {
    display: flex; background: var(--light-gray); border-radius: 8px;
    padding: 3px; border: 1.5px solid var(--border);
  }
  .ld-view-btn {
    padding: 0.32rem 0.7rem; border-radius: 6px; border: none; background: none;
    cursor: pointer; font-size: 0.8rem; font-weight: 600; color: var(--mid); transition: var(--transition);
  }
  .ld-view-btn.active { background: white; color: var(--navy); box-shadow: 0 1px 4px rgba(15,25,35,0.1); }
  ...
  @media (max-width: 480px) {
    .ld-banner h1 { font-size: 1.25rem; }
    .ld-banner-btns { flex-direction: column; }
    .ld-btn { width: 100%; justify-content: center; }
    .ld-stats { gap: 0.6rem; }
    .ld-quick { grid-template-columns: repeat(3, 1fr); gap: 0.5rem; }
...
function StatSkeleton() {
  return (
    <div className="ld-stat" style={{ pointerEvents: 'none' }}>
      <div className="ld-skeleton" style={{ width: 50, height: 50, borderRadius: 12, flexShrink: 0 }} />
      <div style={{ flex: 1 }}>
        <div className="ld-skeleton" style={{ width: 55, height: 26, marginBottom: 8 }} />
        <div className="ld-skeleton" style={{ width: 80, height: 11 }} />
      </div>
    </div>
  );
}
        ...
        <div className="ld-hcard-stats">
          ...
          <div className="ld-hcard-stat">
            <div className="ld-hcard-stat-val" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
              <FaStar style={{ color: '#f5a623', fontSize: '0.75rem' }} />
...
const LandlordDashboard = () => {
  ...
  const stats = [
    { label: 'Active Listings', value: hostels.length,                                                      icon: <FaBuilding />,      color: 'ico-amber' },
    ...
    return (
      <>
        ...
        <div className="ld-auth-loading">
          <FaSpinner style={{ fontSize: '2.2rem', color: '#f5a623', animation: 'spin 0.8s linear infinite' }} />
    ...
    <>
      ...
      <div className={`ld-drawer${drawerOpen ? ' open' : ''}`}>
        ...
        <nav className="ld-drawer-nav">
          ...
          <Link to="/contact" className="ld-drawer-link" onClick={() => setDrawerOpen(false)}><FaBuilding /> Contact</Link>
        </nav>
          ...
          <div className="ld-sec-hd">
            <h2>
              ...
              {!loadingHostels && hostels.length > 0 && (
                <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--mid)', marginLeft: 4 }}>
                  ({hostels.length})
                </span>
              )}
            </h2>
          ...
          {loadingHostels ? (
            ...
            <div className="ld-panel">
              <div className="ld-empty">
                ...
                <p>Start by adding your first property to connect with buyers and tenants across Malawi</p>
                <button className="ld-btn ld-btn-primary" onClick={() => navigate('/hostels/create')}>
                        ...
                        <tr key={hostel._id}>
                          ...
                          <td>
                            <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: 'var(--mid)', fontSize: '0.83rem' }}>
                              <FaMapMarkerAlt style={{ color: 'var(--amber-dark)', flexShrink: 0 }} />
                              {hostel.address}
                            </span>
                          </td>
                          <td style={{ fontWeight: 800, color: 'var(--navy)' }}>MK {hostel.price?.toLocaleString() ?? '—'}</td>
